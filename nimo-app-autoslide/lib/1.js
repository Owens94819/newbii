...
TODO:
FIXE - ACCES item stored on onload event
    */
    ...
    var $obj = {};
    this.delay = 500;
    this.devMode = this.devMode || true;
    var trash = {};
    ...
    var HTMLtag = function(tag, attributes, fallback) {
        var app = App.apps[attributes.app];
        if (!tag) {
...
if (val.indexOf('></')<0) {
val = true;
    fallback.url[attribute.target] = 'value';
}else{
val = false
    fallback.url[attribute.target] = 'innerHTML';
}
        if (fallback) {
            ...
            fallback.url[fallback.url[attribute.target]] =fallback[attribute.view];
            delete(fallback.url[attribute.target])
            for (var prop in fallback.url) {
                if (val == true) {
                    elem.setAttribute(prop, fallback.url[prop])
                }else{
                    elem[prop] = fallback.url[prop]
                }
            }
        }
 ...
 and view attributes before @update could access them
    ...
    */
    ...
    var parseUrl = function(val,x,y) {
         ...
         if (y) {
            if (!val.includes('?')) {
                return ({url:{}})[attribute.view]=val
            }
         }
        if (!val.includes('?')) {
            val = val + "?" + attribute.target + "=innerHTML"
        }
        ...
        if (url == "") {
            var url = attribute.target + "=innerHTML";
        }
        if (!url.includes('=')) {
            var url = url + "="
        }
       ...
       if (!x) {
            url[attribute.target] = url[attribute.target] || 'innerHTML';
            if (view.trim() == 0) {
                view = attribute.com + ':';
...
PROBLEM:
 HEADS UP:
        ...
        */
        ...
        var target = p_y.url[attribute.target];
        ...
        p_u = p_y.url;
        p_u[attribute.app] = x
        p_u[attribute.view] = p_u[attribute.view] || `${p_y[attribute.view]}?${attribute.target}=${target}`;
    ...
    var update = function(apps, ex, ex2) {
        ...
        // var elm = document.querySelectorAll(name);
        var elm = appSelector('app');
        // var arr = [];
        // for (var i = 0; i < elm.length; i++) {
        //   arr.push(elm[i]);
        // }
        //if (arr.length < 1) {
        //   update_ex(App.apps[AppName], true);
        //   return;
        // }
        //console
    ...
    var max_update = function(st, index, length) {
        ...
        //here
        if (p[0]in strapp === false && p[0]in App.apps[AppName] === false) {
            return (false);
        }
...
*/
            new Promise(function(r) {
                ...
                //HEADS up!!
                if (EqualTo(typeof $new, ['string', 'number'])) {
    ...
    var tags = function(elm) {
        ...
        if (val[0] == attribute.com) {
            vl = parseString(vl, null);
            var do_ = syn[vl[0]];
...
if (arguments.length < 1) {
    ...
    App.apps[AppName] = {};
    attribute.action = AppName+attribute.action
    App.apps[AppName][attribute.com] = {};
    var app = App.apps[AppName];
    var $com = App.apps[AppName][attribute.com];
...
return
        ...
        updateItem: function() {
            ...
            */
            if (isClass(this)) {
                var prm = new Promise(function(r) {
                    r(merge(this, `App.apps.${AppName}`, name));
                }
                )
            } else {
                var prm = new Promise(function(r) {
